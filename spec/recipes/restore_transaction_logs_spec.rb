...
describe 'mssqlserver_alwayson::restore_transaction_logs' do
  ...
  it 'passes syntax checks' do
    ...
    databases.each do |db|
      expect(converge).to restore_mssqlserver_alwayson_transaction_log("restore #{db} transaction log").with({
           :database => db,
