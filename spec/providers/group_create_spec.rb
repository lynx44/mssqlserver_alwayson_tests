...
describe 'mssqlserver_alwayson_tests::group_create' do
  group_name = 'group1'
  databases = ['database1']
  ...
  it 'sets expected parameters' do
    expect(converge).to create_mssqlserver_alwayson_group(group_name)
                        .with({
                                  :databases => databases,
  ...
  it 'copies template to cache directory' do
    expect(converge).to create_template("#{Chef::Config[:file_cache_path]}\\CreateAvailabilityGroup.sql")
                        .with({
  ...
  it 'runs connect script on server' do
    expect(converge).to run_mssqlserver_sql_command('create availability group')
                        .with({
                                  :script => "#{Chef::Config[:file_cache_path]}\\CreateAvailabilityGroup.sql",
                                  :database => 'master'
                              })
  end
